...
| 0x40C000-0x411FFF (file 0x20C000-0x211FFF) | **Bosch/ASCET runtime library** in the on-chip flash: 44 interpolation helpers (1-D curve, 2-D map, shared-axis "group" forms, axis search, value-array interpolators) plus saturating arithmetic, debounce counters, ramps and filters | Called 1,343 times from the external-flash application across the region boundary. Every calibration table in the dump is reached through one of them. Full list with argument conventions and data layout: `re/findings/calibration_maps.md`; inventory in `re/calibration_draft.csv` (2026-09-15, B5, issue #19). |
