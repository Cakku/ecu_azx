...
| Which FR name belongs to which `KFPRSOL*` variant (the mode bits of 0x7FB69A were not decoded) | **SETTLED (2026-09-23, G4, calibration_names.md §11.4).** 0x7FB69A is the u16 `bdemod_w` (FR bit coding HOM 0, HMM 1, HOS 2, SCH 3, SKH 4, HSP 6, HKS 7), and four of the six labels in §1 / §3.2 were swapped: **0x5D5224 = `KFPRSOLHKS`** (bit 7), **0x5D53A4 = `KFPRSOLKH`** (bits 4 / 2 / 0x80156F.0), **0x5D54A4 = `KFPRSOLSCH`** (bit 3, with the offset, as the FR's `CWPRSOLAP` bit 5 says), **0x5D52A4 = `KFPRSOLHMM`** (bit 1). `KFPRSOLHOM` and `KFPRSOLOFF` are unchanged. The §1 table and §3.2 pseudo-code keep B9's labels for history; read them with this row |
